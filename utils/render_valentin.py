...
def diffuse_Substance(diffuse, specular):
    return diffuse * (1.0 - specular) / np.pi

def GGX_Substance(roughness, NdotH):
    alpha = torch.square(roughness)
    underD = 1/torch.maximum(torch.tensor([0.001], device='cuda'), (torch.square(NdotH) * (torch.square(alpha) - 1.0) + 1.0))
    return (torch.square(alpha * underD)/np.pi)

def Smith_Substance(roughness, NdotL, NdotV):
    return G1_Substance(NdotL, torch.square(roughness)/2) * G1_Substance(NdotV, torch.square(roughness)/2)
    
def G1_Substance(NdotW, k):
    return 1.0/torch.maximum((NdotW * (1.0 - k) + k), torch.tensor([0.001],  device='cuda'))
...
def render(maps, tex_pos, li_color, camli_pos, gamma=True, isSpecular=False):
	...
	G = Smith_Substance(rough, n_dot_l, n_dot_v)
	...
	if isSpecular:
		f1 = diffuse_Substance(albedo, specular)	
		#f1 = f1*(1-specular)
